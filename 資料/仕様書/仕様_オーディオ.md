...
UpdateSEPlayer[["UpdateSEPlayer(OnpuType)"]]
UpdateBGMManager[["UpdateBGMManager(OnpuType)"]]

PlayBuffer[["PlayBuffer(OnpuType)"]]

if_IsPlay{BGMかSEが再生中か}


AudioPlayer --> START(START)
START --> l_Onpu
l_Onpu --> UpdateSEPlayer

UpdateSEPlayer --> UpdateBGMManager

UpdateBGMManager --> if_IsPlay

if_IsPlay -->|yes| PlayBuffer
if_IsPlay -->|no| EndFlow
...
IsNoteLength{音符の長さが<br>経過したか<br>CurrentNotePos <= 0}
GetNextNote[[GetNextNote<br>次の音符を取り出す]]
setNotePos[SetNotePos<br>音符の長さをセットする]
